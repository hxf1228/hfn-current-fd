function [u, u_hat, omega] = mvmd_raw(signal, opt)
...
if isfield(opt, 'alpha'); alpha = opt.alpha; end
if isfield(opt, 'tau'); tau = opt.tau; end
if isfield(opt, 'K'); K = opt.K; end
if isfield(opt, 'DC'); DC= opt.DC; end
if isfield(opt, 'init'); init= opt.init; end
